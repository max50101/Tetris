...
//Класс написанный на Java, под блоком подразумевается фигура

public class Figure {
    private int shapeIndex; //Форма фигуры
    private int frameNumber;
    private BlockColor color;
    private Point position;

    private Figure(int shapeIndex, BlockColor blockColor) {
        this.frameNumber = 0;
        this.shapeIndex = shapeIndex;
        this.color = blockColor;
        this.position = new Point(FieldConstants.COLUMN_COUNT.getValue() / 2, 0); //cтартовая позиция сверху посередине
    }

    public static Figure createBlock() {
        ...
        Figure block = new Figure(shapeIndex, blockColor);
        block.position.x = block.position.x - Shape.values()
                [shapeIndex].getStartPosition();
        return block;
    }
//enum содержащий доступные цвета фигур
    ...
    //задает блоку конкретный фрэйм и позиция, необходимо для игровых действий
